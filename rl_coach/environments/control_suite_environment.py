...
class ControlSuiteEnvironmentParameters(EnvironmentParameters):
    def __init__(self, level=None):
        ...
        #self.observation_type = ObservationType.Image_and_Measurements
...
class ControlSuiteEnvironment(Environment):
    def __init__(self, level: LevelSelection, frame_skip: int, visualization_parameters: VisualizationParameters,
        ...
        # image observations
        if observation_type != ObservationType.Measurements:
            self.state_space['pixels'] = ImageObservationSpace(shape=self.env.observation_spec()['pixels'].shape, high=255)
        ...
        if observation_type != ObservationType.Image:
            ...
            for observation_space_name, observation_space in self.env.observation_spec().items():
                if len(observation_space.shape) == 0:
                    ...
                    measurements_names.extend(["{}_{}".format(observation_space_name, i) for i in range(observation_space.shape[0])])
            self.state_space['measurements'] = VectorObservationSpace(shape=measurements_space_size, measurements_names=measurements_names)
    ...
    def _update_state(self):
        ...
        if self.observation_type != ObservationType.Measurements:
            self.pixels = self.last_result.observation.get('pixels', 'walker/egocentric_camera')
            self.state['pixels'] = self.pixels  # (240, 320, 3)

        if self.observation_type != ObservationType.Image:
            self.measurements = np.array([])
            self.measurements_cols = []
            for k_sub_obs, sub_observation in self.last_result.observation.items():
                if k_sub_obs not in ['pixels', 'walker/egocentric_camera']:
                    self.measurements_cols.append(k_sub_obs)
    ...
    def get_rendered_image(self, camera_id=0, height=200, width=200):
        return self.env.physics.render(camera_id=camera_id, height=height, width=width)

    def get_num_cameras(self):
        return self.env.physics.model.ncam
